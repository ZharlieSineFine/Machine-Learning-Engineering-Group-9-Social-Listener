...
def _digest_line() -> str:
    ...
    return f"Marketing view&nbsp; ·&nbsp; {now:%a %d %b %Y}"
    ...
    with st.container(border=True):
        ...
        with head_l:
            st.markdown(
                ...
                f'Sentiment trend — {"14 days" if code == "D" else "8 weeks"}</div>',
                unsafe_allow_html=True,
            )
...
def render_alerts(df: pd.DataFrame, n: int = 4) -> None:
    ...
    with st.container(border=True):
        st.markdown(_section_title(_IC_BELL, "Alerts · last batch"), unsafe_allow_html=True)
        if neg.empty:
            st.markdown(
                f'<div style="color:{TEXT_SEC};font-size:13px;">No negative posts in this batch.</div>',
                unsafe_allow_html=True,
            )
            return
        for _, row in neg.head(n).iterrows():
            ...
            st.markdown(
                ...
                f'<span style="background:var(--red-soft);color:{RED};padding:2px 9px;'
                f'border-radius:99px;font-size:10px;font-weight:600;">negative</span>'
                f'{source}{" · " + date_str if date_str else ""}'
                f'</div></div>',
                unsafe_allow_html=True,
            )
...
def render_word_cloud(df: pd.DataFrame) -> None:
    ...
    with st.container(border=True):
        st.markdown(_section_title(_IC_CLOUD, "Top words in negative posts"), unsafe_allow_html=True)
        if not counts:
            st.markdown(
        ...
        st.markdown(
            ...
            f'line-height:1.15;padding-bottom:14px;">{spans}</div>',
            unsafe_allow_html=True,
        )
