...
def _auto_refresh(interval_ms: int = 5000) -> None:
    ...
    the demo. No-op when streamlit-autorefresh isn't installed."""
    try:
...
@st.cache_data(ttl=5)   # short TTL so a new batch shows within seconds during the demo
...
def main() -> None:
    ...
    # KPI tiles + the spike alert summarise the LATEST batch (most recent ingested
    # day); render_timeline() below shows the multi-day trend.
    batch = latest_batch(df)
