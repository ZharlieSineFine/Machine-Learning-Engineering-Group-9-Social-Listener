...
| `dags/evaluate_and_monitor.py` | Dataset `reviews` | Read-only Evidently drift monitor, **data-triggered per batch** off `batch_inference` → `monitoring_reports` + alert. **No auto-retrain** — a human decides whether to run `FORCE_TRAIN=1`. | Charlie, Ha |
